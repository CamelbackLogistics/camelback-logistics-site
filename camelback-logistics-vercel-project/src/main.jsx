...
const services = [
  ...
  },
  {
    ...
    text: "Cost-effective rail options that support high-volume movement while improving efficiency across longer routes."
  },
  {
    icon: "06",
    title: "Hazmat & Food Logistics",
    text: "Specialized handling for hazardous materials and food-grade products with safety, quality, and compliance at the center."
  }
];
...
function validateWebsiteData() {
  return {
    ...
    allServicesHaveTitles: services.every(function (service) {
      return Boolean(service.title) && service.title.length > 3;
    }),
    allServicesHaveCopy: services.every(function (service) {
      return Boolean(service.text) && service.text.length > 20;
    }),
    allServicesHaveTextIcons: services.every(function (service) {
      return /^\d{2}$/.test(service.icon);
    }),
    logoPathIsSet: LOGO_SRC.length > 0,
    videoPathIsSet: VIDEO_SRC.length > 0,
    heroUsesCamelbackName: true,
    hasTestimonials: testimonials.length >= 3,
    testimonialsHaveQuotes: testimonials.every(function (t) {
      return Boolean(t.quote) && t.quote.length > 20;
    })
  };
}
...
console.assert(testResults.logoPathIsSet, "Expected the logo path to be configured.");
console.assert(testResults.videoPathIsSet, "Expected the video path to be configured.");
console.assert(testResults.heroUsesCamelbackName, "Expected the page to represent Camelback Logistics.");
...
function Card(props) {
  const className = props.className || "";
  return <div className={"rounded-3xl " + className}>{props.children}</div>;
}

function SectionLabel(props) {
  const colorClass = props.dark ? "text-rose-900/70" : "text-rose-200/80";
  return <p className={"text-sm uppercase tracking-[0.3em] " + colorClass}>{props.children}</p>;
}
          ...
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
              ...
              <div className="space-y-6 text-lg leading-8 text-slate-300">
                <p>
                  Camelback Logistics partners with clients from procurement through production to final delivery, ensuring every step is optimized for efficiency, transparency, and cost control.
                </p>
                <p>
                  We manage the complexities others avoid — customs clearance, global transportation, regulatory compliance, warehousing, and end-to-end coordination — so our clients can stay focused on growth.
                </p>
                <p>
                  With a strong focus on Southeast Asia and Latin America, we support sourcing, supplier development, and production oversight in some of the world’s most dynamic manufacturing regions. From Vietnam, Thailand, and Indonesia to Mexico, Colombia, and Brazil, we help clients navigate local markets, vet suppliers, manage risk, and ensure consistent product quality.
                </p>
                <p>
                  Our on-the-ground expertise and established relationships allow us to streamline procurement, reduce lead times, and create resilient, diversified supply chains across these key regions.
                </p>
                <p>
                  Backed by a strong network of thoroughly vetted warehousing and transportation partners, we ensure consistent service, reliability, and performance across every touchpoint of the supply chain. Our partners are selected based on strict standards for compliance, safety, capacity, and operational excellence.
                </p>
                <p>
                  Within the United States, we provide robust transportation and warehousing solutions including full truckload (FTL), less-than-truckload (LTL), intermodal, and rail, supported by strategically located 3rd party service providers that enable efficient regional and national coverage. Our facilities support cross-docking, inventory management, cold chain handling, hazardous materials, and value-added services such as kitting and light assembly—ensuring flexibility, speed, and scalability for our clients.
                </p>
                <p>
                  More than a service provider, we act as an extension of your business, offering hands-on consulting and tailored solutions designed around your specific operational needs.
                </p>
              </div>
            </div>
          </div>
